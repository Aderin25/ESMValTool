...
def runs_of_ones_array_spi(bits, spi, spei):
    # make sure all runs of ones are well-bounded
    bounded = np.hstack(([0], bits, [0]))
    # get 1 at run starts and -1 at run ends
    difs = np.diff(bounded)
    run_starts, = np.where(difs > 0)
    run_ends, = np.where(difs < 0)
    spi_sum = np.full(len(run_starts), 0.5)
    ...
    return [run_ends - run_starts, spi_sum, spei_sum]
            ...
            if data_help.count() == 0:
                ...
                return_var[ilat, ilon, 3] = np.mean((spei_sum * events) /
                                                    (np.mean(data_help
                                                             [data_hits])
                                                     * np.mean(events)))
                return_var[ilat, ilon, 4] = np.mean(spi_sum / events)
                return_var[ilat, ilon, 5] = np.mean(spei_sum / events)
...
def plot_map_drought_multi(cfg, data_dict, colormap='jet'):
    ...
    cnplot = plt.contourf(lons, lats, spi, spei
...
def plot_time_series_drought(cfg, cube, add_to_filename=''):
    """Plot time series."""
    ...
    axx.plot_date(time, spi, spei, '-', tz=None, xdate=True, ydate=False,
    ...
    axx.set_xlabel('Time')
    axx.set_ylabel('SPI', 'SPEI')
    axx.set_title('Mean SPI ' + dataset_name + ' ' + add_to_filename, 
                  'Mean SPEI ' + dataset_name + ' ' + add_to_filename)
