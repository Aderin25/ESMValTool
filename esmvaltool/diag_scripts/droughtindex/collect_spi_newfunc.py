...
def runs_of_ones_array_spi(bits, spi):
    # make sure all runs of ones are well-bounded
    bounded = np.hstack(([0], bits, [0]))
    # get 1 at run starts and -1 at run ends
    difs = np.diff(bounded)
    run_starts, = np.where(difs > 0)
    run_ends, = np.where(difs < 0)
    spi_sum = np.full(len(run_starts), np.nan)
    for iii, indexs in enumerate(run_starts):
        spi_sum[iii] = np.sum(spi[indexs:run_ends[iii]])

    return [run_ends - run_starts, spi_sum]
...
def count_spells(data, threshold, axis):
    ...
    # spell_point_average_spi = np.zeros((listshape[0], listshape[1]))
    for ilat in range(listshape[0]):
        for ilon in range(listshape[1]):
            ...
            if data_help.count() == 0:
                ...
                [events, spi_sum] = runs_of_ones_array_spi(data_hits,
                                                           data_help)

                return_var[ilat, ilon, 0] = np.count_nonzero(events)
                # np.sum(np.isfinite(events))
                return_var[ilat, ilon, 1] = np.mean(events)
                return_var[ilat, ilon, 2] = np.mean((spi_sum * events) /
                                                    (np.mean(data_help
                                                             [data_hits])
                                                     * np.mean(events)))
                return_var[ilat, ilon, 3] = np.mean(spi_sum / events)
...
def plot_histogram_spi_multi(cfg, data_dict):
    """Plot histogram for multi models and mean."""

    # Reshape array
    data = data_dict['data']
    sh1, sh2, sh3, sh4 = data.shape
    rdata = np.reshape(data,(sh1*sh2*sh3, sh4))
    fig, axx = plt.subplots(figsize=(8, 4))
    colors_ini = ['blue', 'red', 'tan', 'lime', 'orange', 'darkgreen', 'black',
              'darkorchid', 'grey', 'cornflowerblue', 'indigo', 'turquoise',
              'saddlebrown','maroon','coral','violet','deepskyblue']
    print("len(data_dict[modelnames)")
    print(len(data_dict['modelnames']))
    colors = colors_ini[0:len(data_dict['modelnames'])]
    axx.hist(rdata, bins=data_dict['bins'], density=True, histtype='bar',
                 color=colors, label=data_dict['modelnames'])
    ...
    axx.set_xlabel('SPI')
    axx.set_ylabel(' ')
    axx.set_title(data_dict['model_kind'] + ' ' + str(data_dict['start']) +
                  '-' + str(data_dict['end']))
    axx.set_ylim([0, 0.7])
    ...
    axx.legend(loc=0)
    fig.tight_layout()
    print(os.path.join(cfg[n.PLOT_DIR],
                             'spi_' + data_dict['filename'] + '.' +
...
def plot_map_spi_multi(cfg, data_dict, colormap='jet'):
    """Plot contour maps for multi model mean."""
    print("data_dict['data']")
    print(data_dict['data'])
    mask = np.isnan(data_dict['data'])
    spi = np.ma.array(data_dict['data'], mask=mask)
    # np.ma.masked_less_equal(spi, 0)
    print("mask")
    print(mask)
    print("spi 1")
    print(spi)
    ...
    cnplot = plt.contourf(lons, lats, spi,
...
def plot_map_spi(cfg, cube, levels, add_to_filename='', name=''):
    """Plot contour map."""
    print("hello map 1")
    # SPI array to plot
    # spi = cube.data
    ...
    spi = np.ma.array(cube.data, mask=mask)
    np.ma.masked_less_equal(spi, 0)
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(cfg[n.PLOT_DIR],
                             'spi_map' + add_to_filename + '_' +
                             dataset_name + '.' +
                             cfg[n.OUTPUT_FILE_TYPE]))
    plt.close()


def plot_time_series_spi(cfg, cube, add_to_filename=''):
    """Plot time series."""
    ...
    axx.plot_date(time, spi, '-', tz=None, xdate=True, ydate=False,
    ...
    axx.set_xlabel('Time')
    axx.set_ylabel('SPI')
    axx.set_title('Mean SPI ' + dataset_name + ' ' + add_to_filename)
    ...
    # Save plot to file
    fig.savefig(os.path.join(cfg[n.PLOT_DIR],
                             'spi_line' + add_to_filename + '_' + dataset_name
                             + '.' +
                             cfg[n.OUTPUT_FILE_TYPE]))
    plt.close()
