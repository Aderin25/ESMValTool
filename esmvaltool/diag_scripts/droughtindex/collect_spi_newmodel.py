...
from esmvaltool.diag_scripts.droughtindex.collect_spi_newfunc import (
    count_spells, plot_map_spi_multi, plot_map_spi,
    plot_histogram_spi_multi)
...
def main(cfg):
    ...
    for iii, spi_file in enumerate(glob.iglob(input_filenames)):
        ...
        if lasttime > timecheck:
            ...
            print(locals().keys())
            #print(locals().values())
            if 'all_spi_hist' in locals().keys():
                all_spi_hist[:, :, :, iii] = tscube.data
            else:
                shape_histo = tscube.data.shape + (len(ncfiles),)
                all_spi_hist = np.full(shape_histo, np.nan)
                all_spi_hist[:, :, :, iii] = tscube.data
            ...
            print("tscube.data.shape hist")
            print(tscube.data.shape)
            if 'all_spi_ssp585' in locals().keys():
                all_spi_ssp585[:, :, :, iii] = tscube.data
            else:
                shape_histo = tscube.data.shape + (len(ncfiles),)
                all_spi_ssp585 = np.full(shape_histo, np.nan)
                all_spi_ssp585[:, :, :, iii] = tscube.data
            ...
            cube2.data = drought_show.data[:, :, 3]
            plot_map_spi(cfg, cube2, drought_numbers_level,
                         add_to_filename='SSP585_Avr_spi_of_Events',
                         name='SSP585_Average spi of Events')
    ...
    histo_dict['filename'] = 'Historic_histoplot'
    histo_dict['bins'] = [-3.0, -2.0, -1.0, 0,
              1, 2, 3]
    plot_histogram_spi_multi(cfg, histo_dict)
    
    
    histo_dict['data'] = all_spi_ssp585
    histo_dict['model_kind'] = 'SSP585'
    histo_dict['start'] = start_year_future
    histo_dict['end'] = end_year_future
    histo_dict['filename'] = 'SSP585_histoplot'
    plot_histogram_spi_multi(cfg, histo_dict)
    ...
    data_dict['drought_char'] = 'Average SPI of Events'
    data_dict['filename'] = 'SSP585_Avr_spi_of_Events'
    data_dict['drought_numbers_level'] = np.arange(-2.8, -1.8, 0.2)
    plot_map_spi_multi(cfg, data_dict, colormap='gnuplot')
    ...
    data_dict['drought_char'] = 'Average SPI of Events [%]'
    data_dict['filename'] = 'Percentage_difference_of_Avrspi_of_Events'
    data_dict['drought_numbers_level'] = np.arange(-50, 60, 10)
    plot_map_spi_multi(cfg, data_dict, colormap='rainbow')
    ...
    plot_map_spi_multi(cfg, data_dict, colormap='cool')
    ...
    data_dict['drought_char'] = 'Standard deviation of the Average SPI of Events'
    data_dict['filename'] = 'Historic_Average_spi_of_Events_std'
    ...
    data_dict['drought_char'] = 'Standard deviation of the Average SPI of Events'
    data_dict['filename'] = 'SSP585_Average_spi_of_Events_std'
    data_dict['drought_numbers_level'] = np.arange(0, 0.5, 0.05)
    plot_map_spi_multi(cfg, data_dict, colormap='cool')
    ...
    data_dict['drought_char'] = 'Standard deviation of the Severity Index of Events [%]'
    data_dict['filename'] = 'Percentage_difference_of_Sev_index_of_Events_std'
    data_dict['drought_numbers_level'] = np.arange(-200, 210, 10)
    plot_map_spi_multi(cfg, data_dict, colormap='rainbow')

    data_dict['data'] = perc_diff_std[:, :, 3]
    data_dict['drought_char'] = 'Standard deviation of the Average SPI of Events [%]'
    data_dict['filename'] = 'Percentage_difference_of_Avrspi_of_Events_std'
    data_dict['drought_numbers_level'] = np.arange(-200, 210, 10)
    plot_map_spi_multi(cfg, data_dict, colormap='rainbow')
