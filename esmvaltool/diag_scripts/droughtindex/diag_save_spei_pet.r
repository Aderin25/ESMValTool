...
for (vnr in 1:nrvars){
  ...
  if (var_input[1][[1]]$short_name == "evspsblpot") {
    pet_input <- var_input
    modfilepet <- names(metadata)
  }
}
...
ref2 <- getnc(pet_input, nref, lat = FALSE)
...
for (mod in 1:nmods){
   ...
   xprov$ancestors <- list(modfile1[mod], modfilepet[mod])
